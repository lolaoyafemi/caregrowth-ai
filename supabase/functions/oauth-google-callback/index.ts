...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Store connection in Supabase (server-side persistence to work without frontend login)
    ...
    try {
      // Prefer user_profiles (primary source)
      ...
      if (profileByEmail?.user_id) {
        ...
        // Fallback to users table by email
      ...
      if (linkedUserId) {
        // Ensure an agency exists for this user (admin_user_id)
        ...
        if (existingAgency?.id) {
          ...
          if (agencyCreateError) {
            console.error('Failed creating agency for user:', agencyCreateError.message);
          } else if (createdAgency?.id) {
            agencyId = createdAgency.id as string;
          }
        }

        // Upsert google connection for this user
        if (agencyId) {
          ...
          if (existingConn?.id) {
            ...
            if (updateErr) console.error('Failed updating google connection:', updateErr.message);
          } else {
            ...
            if (insertErr) console.error('Failed inserting google connection:', insertErr.message);
          }
        }
      } else {
        console.warn('No matching application user found for Google email:', userInfo.email);
      }
    } catch (persistErr) {
      console.error('Error while persisting Google connection server-side:', persistErr);
    }
