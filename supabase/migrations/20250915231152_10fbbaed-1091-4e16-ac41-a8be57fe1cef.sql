...
-- Add security logging function for payment modifications
CREATE OR REPLACE FUNCTION public.log_payment_modification()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
  -- Log payment data modifications for security monitoring
...
-- Create trigger to log payment modifications for security monitoring
DROP TRIGGER IF EXISTS log_payment_modification_trigger ON public.payments;
CREATE TRIGGER log_payment_modification_trigger
  AFTER INSERT OR UPDATE OR DELETE ON public.payments
  FOR EACH ROW
  EXECUTE FUNCTION public.log_payment_modification();